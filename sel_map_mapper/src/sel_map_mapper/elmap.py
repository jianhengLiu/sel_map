...
class Map:
    ...
    def __init__(self, mesh, camera=None, mesh_topic='mesh', mat_service='get_materials', enableMat=False, saveOpts=SaveOptions(), thresholdElemToMove=1, threadRate=20, threaded=True, mapCameraPosePublisher=None):
        ...
        if saveOpts.dir is not None:
            ...
            Path(os.path.join(saveOpts.dir, 'vertices/')
                 ).mkdir(parents=True, exist_ok=True)
            Path(os.path.join(saveOpts.dir, 'faces/')
                 ).mkdir(parents=True, exist_ok=True)
            if saveOpts.classifications:
                Path(os.path.join(saveOpts.dir, 'classes/')
                     ).mkdir(parents=True, exist_ok=True)
            if saveOpts.confidence:
                Path(os.path.join(saveOpts.dir, 'confidence/')
                     ).mkdir(parents=True, exist_ok=True)
        ...
        self.pub_costs = rospy.Publisher(
            mesh_topic + '/costs', MeshVertexCostsStamped, queue_size=10)
        self.pub_camera = None
        if mapCameraPosePublisher is not None:
            self.pub_camera = rospy.Publisher(
                mapCameraPosePublisher, PoseStamped, queue_size=10)
        import tf2_ros
        ...
        self.meshMsg.header = Header(
            frame_id='sel_map', stamp=rospy.Time.now())
        # self.meshMsg.mesh_geometry.faces = [MeshTriangleIndices(idxs) for idxs in self.simplices]
        self.pub_mesh = TriangularMeshPublisher(
            mesh, self.uuid, "sel_map", mesh_topic)
        ...
        if enableMat:
            # ROS service
            if not threaded:
                self.mesh_mat_service = rospy.Service(
                    mat_service, GetMaterials, self.materialCallback)
            # Preprocess material colors
            self.materials = MeshMaterials()
            import rospkg
            rospack = rospkg.RosPack()
            path = rospack.get_path('sel_map')
            property_file = rospy.get_param("terrain_properties", os.path.join(
                path, "config/semseg_properties/csail_semseg_properties.yaml"))
            ...
            if colorscale_args is None:
                with open(os.path.join(path, "config/colorscales/default.yaml")) as file:
                    colorscale_args = yaml.load(file, Loader=yaml.FullLoader)[
                        'colorscale']
            ...
            if colorscale.bypass:
                ...
                materials = [ColorRGBA(
                    *colorscale.mapToColor(prop_dict[key]['friction']['mean']), 1) for key in keys_list]
            self.materials.materials = [MeshMaterial(
                color=materials[i], has_texture=False) for i in range(len(materials))]
        ...
        if threaded:
            ...
            self.daemon_thread = threading.Thread(
                target=self.threadedRoutine, args=[mat_service])
    ...
    def shiftIfNeeded(self, pose):
        ...
        # shift the mesh
        elementShift = np.ceil(
            (np.abs(displacement[:2]) - threshold) / self.mesh.elementLength)
    ...
    def update(self, pose, rgbd, intrinsic=None, R=None, min_depth=0, max_depth=5.0):
        ...
        points = self.camera.getProjectedPointCloudWithLabels(
            intrinsic=intrinsic, R=R, min_depth=min_depth, max_depth=max_depth)
        # Shift and rotate as needed
        points[:, :3] = pose.location + \
            np.dot(points[:, :3], np.transpose(pose.rotation))
        ...
        if self.pub_camera is not None:
            ...
            # - self.frame.origin.location[0]
            poseMsg.pose.position.x = pose.location[0]
            # - self.frame.origin.location[1]
            poseMsg.pose.position.y = pose.location[1]
            # - self.frame.origin.location[2]
            poseMsg.pose.position.z = pose.location[2]
            rot = np.identity(4)
    ...
    def updateByPCL(self, pose, pcl, min_depth=0, max_depth=5.0):
        if self.crash:
            raise Exception('Map Daemon Thread Crashed!')
        # Shift the mesh if we need to
        self.shiftIfNeeded(pose)
        ...
        # Shift and rotate as needed
        pcl[:, :3] = pose.location + \
            np.dot(pcl[:, :3], np.transpose(pose.rotation))

        # Transform to map origin
        pcl[:, :3] = pcl[:, :3] - self.origin.location
        points = np.column_stack([pcl, np.zeros(pcl.shape[0])])

        mask = points[:, 2] < 1.0  # boolean masking is slooooooow
        points = points[mask]
        ...
        with self.lock:
            if self.save:
                ...
                vertex_file = os.path.join(
                    self.saveOpts.dir, 'vertices/' + str(self.frame.id) + '.npy')
                simplex_file = os.path.join(
                    self.saveOpts.dir, 'faces/' + str(self.frame.id) + '.npy')
                ...
                if self.saveOpts.classifications:
                    classes_file = os.path.join(
                        self.saveOpts.dir, 'classes/' + str(self.frame.id) + '.npy')
                    np.save(classes_file, self.pub_mesh.get_onehot_classes(
                        self.classes, passthrough=False))
                if self.saveOpts.confidence:
                    confidence_file = os.path.join(
                        self.saveOpts.dir, 'confidence/' + str(self.frame.id) + '.npz')
            ...
            if self.publish:
                ...
                if self.enableMat:
                    ...
                    simplex_colors = self.pub_mesh.get_onehot_classes(
                        self.classes, passthrough=False)

                    # not actually clustered, just for viz
                    clustered_faces = [MeshFaceCluster(face_indices=np.argwhere(simplex_colors == i).flatten(
                    ).astype(np.uint32)) for i in self.materials.cluster_materials]
                ...
                cost[cost < -1] = -1
                self.heightCosts.mesh_vertex_costs = MeshVertexCosts(
                    costs=cost)
                import time
                tic = time.perf_counter()
