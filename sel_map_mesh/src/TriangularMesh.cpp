...
TriangularMesh::TriangularMesh(const double bounds[3], const double originHeight,
                               double elementLength, unsigned int pointLimit, float heightPartition,
                               double heightSafetyCheck, unsigned int terrainClasses,
                               bool groundTruth, bool verbose, unsigned int seed)
    : Mesh<TriangleElement>(originHeight, bounds, elementLength, pointLimit, heightPartition,
                            heightSafetyCheck, seed),
...
inline Eigen::Array<unsigned int, 3, 1> TriangularMesh::genSimplices(unsigned int triangle)
{
    ...
    auto LR    = triangle % 2;
    return (Eigen::Array<unsigned int, 3, 1>() << unravelIndex(row + LR, col + LR),
            unravelIndex(row + 1 - LR, col + LR), unravelIndex(row + LR, col + 1 - LR))
        .finished();
}
...
#pragma omp parallel for schedule(static, 8)
    for (int el_idx = 0; el_idx < num_elements; ++el_idx)
    {
        ...
        dirichletParamsForTerrainClass[el_idx] =
            elementsVec[el_idx]->dirichletParamsForTerrainClass.data();
    }
}

void TriangularMesh::advance(const Eigen::Ref<const RowArray_t, Eigen::Aligned16> &points,
                             double                                                z_height)
{
    ...
    Eigen::Map<IndexArray_t>(updateVert.data(), updateElem.size(), 3) =
        simplices(updateElem, Eigen::all);
    auto vec_end = std::unique(
        updateVert.begin(),
        updateVert.end());  // 去重：https://blog.csdn.net/quchen528/article/details/89413213
    ...
    else
    {
        ...
        auto res = std::set_union(activeElements.begin(), activeElements.end(), updateElem.begin(),
                                  updateElem.end(), temp.begin());
...
#pragma omp parallel for schedule(static, 8)
    ...
    {
        trimmedDirichletParams[pos] =
            elementsVec[activeElements[pos]]->dirichletParamsForTerrainClass.data();
    }
...
Eigen::ArrayXi
TriangularMesh::generatePointBinning(const Eigen::Ref<const RowArray_t, Eigen::Aligned16> &points)
{
    ...
    auto stage1_bins = calc_y.cast<int>() * (verticesLength[0] - 1) * 2 + calc_x.cast<int>() * 2 +
                       (delta_x + delta_y).cast<int>();
    ...
    binning          = (outbounds_x || outbounds_y || outbounds_z)
                  .select(Eigen::VectorXi::Constant(points.rows(), -1), stage1_bins);
    return binning;
}

std::vector<unsigned int>
TriangularMesh::addPointsToMesh(const Eigen::Ref<const RowArray_t, Eigen::Aligned16> &points)
{
    ...
    for (unsigned int point_idx = 0; point_idx < points.rows(); ++point_idx)
    {
        ...
        if (bins(point_idx) == -1)
            continue;
        // Push to the sparse list; // new points?
        if (indices[bins(point_idx)].size() == 0)
            elemToUpdate.push_back(bins(point_idx));
        // Push to the index list // existed points?
...
#pragma omp for schedule(dynamic, 4)
        for (unsigned int sparse_el_idx = 0; sparse_el_idx < elemToUpdate.size(); ++sparse_el_idx)
        {
            ...
            if (classpoints)
                element.addSegmentation(indices[el_idx], points, gens[threadId], one_hot);
    ...
    for (auto sparse_idx_vec : elemToUpdate)
    {
        ...
        this->points.middleRows(start, size) =
            points(indices[sparse_idx_vec], Eigen::seqN(Eigen::fix<0>(), Eigen::fix<4>()));
        start += size;
    }
    updateTree = true;
    return elemToUpdate;
...
#pragma omp parallel for schedule(dynamic, 2)
    for (unsigned int vert_idx = 0; vert_idx < vertToUpdate.size(); ++vert_idx)
    {
        ...
        auto closest_points =
            Mesh<TriangleElement>::getClosestPoints(vertex.head<2>(), AllPoints, radius, false);
        ...
        for (unsigned int row_idx = 0; row_idx < closest_points.first.rows(); ++row_idx)
        {
            ...
            auto var =
                row(3) * row(3) *
                (1 + sqr_dist / elementLength / elementLength + 2 * sqrt(sqr_dist) / elementLength);
...
#pragma omp parallel for
    ...
    // Shift
    vertexVector.rightCols<2>().middleRows(res_start, res_end - res_start) =
        cachedHeights.middleRows(start, end - start);

    // Reset new vertices
    auto z_mu = vertexVector.col(2).reshaped<Eigen::RowMajor>(verticesLength[1], verticesLength[0]);
    auto z_sigma =
        vertexVector.col(3).reshaped<Eigen::RowMajor>(verticesLength[1], verticesLength[0]);
...
#pragma omp parallel for schedule(static,                                                          \
                                  8)  // reduction(shifted_active_elements: new_activeElements)
    for (int el_idx = 0; el_idx < elementsVec.size(); ++el_idx)
    {
        auto &element      = *(elementsVec[el_idx]);
        ...
        dirichletParamsForTerrainClass[element.index] =
            element.dirichletParamsForTerrainClass.data();
        new_vec[element.index] = &element;

        // reset the element if it's been wrapped around
        if (new_row < 0 || new_col < 0 || new_row >= verticesLength[1] - 1 ||
            new_col >= verticesLength[0] - 1)
        {
            element.reset();
...
#pragma omp parallel for schedule(static, 64)
    ...
    activeElements.erase(std::remove(activeElements.begin(), activeElements.end(), num_elements),
                         activeElements.end());
